...
def verify(be_root: Path | None) -> None:
    ...
    require(names == ["V001__create_learner_and_settings.sql", "V002__seed_default_settings.sql", "V003__create_admin_settings_audit.sql", "V004__create_settings_selection_delivery.sql", "V005__create_keyword_catalog.sql", "V006__create_learning_result_journal.sql", "V007__create_level_test.sql"], "migration 목록 불일치")
    ...
    selection = (migration / names[3]).read_text(encoding="utf-8")
    selection_sql = "\n".join(line for line in selection.splitlines() if not line.strip().startswith("--"))
    require(re.findall(r"CREATE TABLE (\w+)", selection_sql) == ["language_learning_settings_selection_delivery"], "V004 범위 불일치")
    require(not re.search(r"\b(DROP|ALTER|TRUNCATE|INSERT)\b|DELETE\s+FROM|UPDATE\s+\w+\s+SET", selection_sql, re.I), "V004는 수신 이력 테이블만 추가해야 합니다.")
    require("REFERENCES language_learning_learner (user_id)" in selection_sql and "translacat." not in selection_sql, "수신 이력 FK는 LL 내부여야 합니다.")
    service_routes = (MAIN / "features/settings/api/SettingsServiceRoutes.kt").read_text(encoding="utf-8")
    require("authenticate(SETTINGS_SERVICE_AUTH)" in service_routes and service_routes.count('get("/internal/v1/') == 5, "서비스 조회 경계 불일치")
    require('tokenUse", "ll-settings-service"' in (MAIN / "bootstrap/InternalAuthentication.kt").read_text(encoding="utf-8"), "서비스 토큰 목적 검증 누락")
    require((ROOT / "build.gradle.kts").read_text(encoding="utf-8").count('"**/SettingsCutoverIntegrationTest*"') == 2, "통합 테스트 분리 누락")
    print("PASS: V004 additive receiver, five service-only reads and opt-in integration tests")
