...
class KeywordOperationsTest {
    ...
    @Test fun `빈 카탈로그와 개인 목록을 안전하게 반환한다`() { runBlocking {
    ...
    @Test fun `처음 학습하기 전에는 커스텀 키워드가 즉시 활성화된다`() { runBlocking {
    ...
    @Test fun `학습 시작 후 새 커스텀 키워드는 다음 날 후보가 된다`() { runBlocking {
    ...
    @Test fun `미학습자의 GET은 미래 예약도 즉시 승격한다`() { runBlocking {
        val created = service.createCustom(123, true, topic())
    ...
    @Test fun `커스텀 수정 화면과 오늘 문제 생성의 텍스트를 분리한다`() { runBlocking {
    ...
    @Test fun `수정에서 생략된 부모는 원본대로 연결을 해제한다`() { runBlocking {
        val parent = service.createSystem(900, topic())
        val id = service.createCustom(123, false, KeywordChange(text = "deploy", type = KeywordType.VOCABULARY, parentKeywordId = parent.id)).id
    ...
    @Test fun `다른 사용자의 커스텀 키워드를 변경하거나 비활성화할 수 없다`() { runBlocking {
        val id = service.createCustom(123, false, topic()).id
        assertEquals("KEYWORD_NOT_FOUND", assertFailsWith<LearningBusinessException> {
    ...
    @Test fun `중복은 비활성 및 예약된 텍스트까지 검사하고 사용자별로 구분한다`() { runBlocking {
        val id = service.createCustom(123, false, topic()).id
        service.updateCustom(123, true, id, KeywordChange(text = "New"))
        assertEquals("KEYWORD_DUPLICATED", assertFailsWith<LearningBusinessException> { service.createCustom(123, true, topic("Ｎｅｗ")) }.code)
        service.createCustom(456, false, topic("New"))
        assertEquals(2, db.customRows.size)
    } }
    @Test fun `삭제는 물리 삭제가 아닌 활성값 기준 예약 비활성화다`() { runBlocking {
        val id = service.createCustom(123, false, topic()).id
        service.updateCustom(123, true, id, KeywordChange(text = "New"))
    ...
    @Test fun `시스템 선택은 사용자당 한 행이며 최초 availableFrom은 유지한다`() { runBlocking {
    ...
    @Test fun `비활성 시스템 키워드는 일반 목록 후보 선택에서 제외한다`() { runBlocking {
        val keyword = service.createSystem(900, topic())
        service.selectSystem(123, false, keyword.id, true)
    ...
    @Test fun `시스템 어휘는 활성 root 토픽 부모가 필요하고 3단계는 거부한다`() { runBlocking {
        assertFailsWith<LearningBusinessException> { service.createSystem(900, KeywordChange(text = "deploy", type = KeywordType.VOCABULARY)) }
        val parent = service.createSystem(900, topic())
        val word = service.createSystem(900, KeywordChange(text = "deploy", type = KeywordType.VOCABULARY, parentKeywordId = parent.id))
        assertFailsWith<LearningBusinessException> { service.createCustom(123, false, KeywordChange(text = "child", type = KeywordType.VOCABULARY, parentKeywordId = word.id)) }
        assertFailsWith<LearningBusinessException> { service.updateSystem(900, parent.id, KeywordChange(parentKeywordId = parent.id)) }
    } }
    @Test fun `자식이나 커스텀 예약 참조가 있는 토픽은 비활성화할 수 없다`() { runBlocking {
        val parent = service.createSystem(900, topic())
        val other = service.createSystem(900, topic("Business"))
        val custom = service.createCustom(123, false, KeywordChange(text = "deploy", type = KeywordType.VOCABULARY, parentKeywordId = parent.id))
        service.updateCustom(123, true, custom.id, KeywordChange(parentKeywordId = other.id))
        assertFailsWith<LearningBusinessException> { service.updateSystem(900, parent.id, KeywordChange(active = false)) }
        assertFailsWith<LearningBusinessException> { service.updateSystem(900, other.id, KeywordChange(active = false)) }
        assertTrue(db.systemRows.getValue(parent.id).active); assertTrue(db.systemRows.getValue(other.id).active)
    } }
    @Test fun `시스템 중복 및 음수 정렬 순서를 거부한다`() { runBlocking {
        service.createSystem(900, topic())
        assertFailsWith<LearningBusinessException> { service.createSystem(900, topic("ＩＴ")) }
        assertFailsWith<LearningBusinessException> { service.createSystem(900, topic("Other").copy(sortOrder = -1)) }
        assertEquals(1, db.systemRows.size)
    } }
    @Test fun `커스텀 어휘에는 부모가 없어도 된다`() { runBlocking {
        assertNull(service.createCustom(123, false, KeywordChange(text = "deploy", type = KeywordType.VOCABULARY)).parentKeywordId)
    } }
    @Test fun `잘못된 수정은 앞선 pending 승격도 같은 키워드 트랜잭션에서 롤백한다`() { runBlocking {
        val id = service.createCustom(123, true, topic()).id
        val before = db.customRows.getValue(id)
        date = today.plusDays(1)
        assertFailsWith<LearningBusinessException> { service.updateCustom(123, true, id, KeywordChange(text = " ")) }
        assertEquals(before, db.customRows.getValue(id))
    } }
    @Test fun `저장 실패는 새로운 learner와 키워드를 남기지 않는다`() { runBlocking {
        db.failWrites = true
    ...
    @Test fun `시스템은 정렬 순서와 ID 커스텀은 ID로 반환한다`() { runBlocking {
    ...
    @Test fun `필수 값과 canonical 길이를 검증한다`() { runBlocking {
        for (input in listOf(KeywordChange(), topic(" "), topic("x".repeat(201)), topic().copy(canonicalKey = "x".repeat(201)))) {
