...
class AdminSettingsLifecycleTest {
    @Test fun `관리자 변경과 before after 감사는 동일 범위에서 처리된다`() = runBlocking {
        val work=FakeAdminWork();val result=UpdateAdminSettings(work).execute(987,AdminSettingsChange(dailyKeywordMaxCount=8))
    ...
    @Test fun `검증 실패면 저장 및 감사가 모두 발생하지 않는다`() = runBlocking {
        val work=FakeAdminWork()
        assertFailsWith<LearningBusinessException> {UpdateAdminSettings(work).execute(987,AdminSettingsChange(dailyKeywordMaxCount=21))}
        assertEquals(F.admin(),work.current);assertTrue(work.audit.isEmpty())
    }
    @Test fun `감사 실패시 관리자 변경도 롤백된다`() = runBlocking {
        val work=FakeAdminWork().apply {failAudit=true}
        assertFailsWith<IllegalStateException> {UpdateAdminSettings(work).execute(987,AdminSettingsChange(dailyKeywordMaxCount=8))}
        assertEquals(F.admin(),work.current)
    }
    @Test fun `빈 변경도 감사는 기록하되 관리자 값은 그대로다`() = runBlocking {
    ...
    @Test fun `관리자 식별자는 양수만 허용한다`() = runBlocking {
        val work=FakeAdminWork()
    ...
    private class FakeAdminWork : AdminSettingsUnitOfWork {
        var current=F.admin();var failAudit=false
        ...
        override suspend fun <T> execute(block:AdminSettingsTransaction.()->T):T {
            val before=current;val size=audit.size
            try {
                return block(object:AdminSettingsTransaction {
                    ...
                    override val settings=object:AdminSettingsRepository {
                        ...
                        override fun save(settings:AdminSettings,adminUserId:Long,nowUtc:LocalDateTime)=settings.also {current=it}
            ...
            } catch(failure:Throwable) {current=before;while(audit.size>size)audit.removeAt(audit.lastIndex);throw failure}
        }
    }
}
