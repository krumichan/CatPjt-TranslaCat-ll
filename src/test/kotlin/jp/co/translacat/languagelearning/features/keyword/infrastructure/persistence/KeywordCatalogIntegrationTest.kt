...
class KeywordCatalogIntegrationTest {
    ...
    private fun sql(db: LocalScratchMysql, sql: String) {
        db.connect().use { connection -> connection.createStatement().use { it.executeUpdate(sql) } }
    }
    ...
    @Test
    fun `V004에서 V005로 올려도 Settings와 수신 이력을 변경하지 않는다`() {
        LocalScratchMysql.use { db ->
    ...
    @Test
    fun `빈 DB에는 키워드 데이터가 없고 생성 및 예약 승격을 재조회한다`() {
        db { db, _, ops ->
            runBlocking {
    ...
    @Test
    fun `두 pool에서 동시에 같은 사용자 키워드를 생성해도 한 행만 남는다`() {
        db { db, _, ops ->
            DatabaseFactory(db.settings()).use { other ->
                runBlocking {
    ...
    @Test
    fun `DB collation unique 위반은 업무 중복 오류로 변환한다`() {
        db { db, _, ops ->
            runBlocking {
    ...
    @Test
    fun `실제 FK는 부모 삭제와 Core 사용자 참조를 허용하지 않는다`() {
        db { db, _, ops ->
            runBlocking {
    ...
    @Test
    fun `비활성 learner를 다시 활성화하거나 키워드를 만들지 않는다`() {
        db { db, _, ops ->
            runBlocking {
    ...
    @Test
    fun `실제 locale 매핑과 부모 예약 참조 보호를 확인한다`() {
        db { db, _, ops ->
            runBlocking {
                val parent = ops.createSystem(900, KeywordChange(text = "IT", type = KeywordType.TOPIC))
                val id = parent.id
