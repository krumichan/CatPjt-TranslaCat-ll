...
class SettingsTableMappingTest {
    ...
    fun `네 테이블의 컬럼과 null 허용 여부가 적용된 V001과 일치한다`() {
        val sql =
            checkNotNull(javaClass.getResourceAsStream("/db/migration/V001__create_learner_and_settings.sql")).bufferedReader()
                .use { it.readText() }
        ...
        for (table in tables) {
            ...
            val expected =
                Regex("(?m)^    (\\w+) (?:BIGINT|INT|DOUBLE|BOOLEAN|VARCHAR\\(\\d+\\)|DATETIME\\(6\\)|DATE) (.*)").findAll(
                    body
