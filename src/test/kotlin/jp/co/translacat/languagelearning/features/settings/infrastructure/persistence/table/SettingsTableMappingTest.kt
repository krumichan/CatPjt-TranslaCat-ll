...
class SettingsTableMappingTest {
    ...
    fun `네 테이블의 컬럼과 null 허용 여부가 적용된 V001과 일치한다`() {
        val sql = checkNotNull(javaClass.getResourceAsStream("/db/migration/V001__create_learner_and_settings.sql"))
            .bufferedReader().use { it.readText() }
        val tables: List<Table> = listOf(LearnersTable, UserSettingsTable, AdminSettingsTable, ListeningPoliciesTable)
        for (table in tables) {
            val body = Regex("CREATE TABLE " + table.tableName + " \\((.*?)\\) ENGINE=", RegexOption.DOT_MATCHES_ALL)
                .find(sql)?.groupValues?.get(1) ?: error("스키마에 테이블이 없습니다: ${table.tableName}")
            val expected = Regex("(?m)^    (\\w+) (?:BIGINT|INT|DOUBLE|BOOLEAN|VARCHAR\\(\\d+\\)|DATETIME\\(6\\)|DATE) (.*)")
                .findAll(body).associate { it.groupValues[1] to !it.groupValues[2].contains("NOT NULL") }
