...
class ListeningTaskSelectionPolicyTest {
    @Test fun `32개 모든 조합에서 BE의 8개 허용 집합만 통과한다`() {
        val expected = setOf(setOf(DICTATION),setOf(REPEAT_AFTER_AUDIO),setOf(DICTATION,INTERPRETATION),
            setOf(DICTATION,REPEAT_AFTER_AUDIO),setOf(INTERPRETATION,REPEAT_AFTER_AUDIO),
            setOf(DICTATION,INTERPRETATION,REPEAT_AFTER_AUDIO),setOf(COMPREHENSION),setOf(SUMMARY))
    ...
    @Test fun `중복과 null을 무시하고 통과시키지 않는다`() {
    ...
    @Test fun `정상 입력은 enum 순서의 공백 없는 JSON으로 저장한다`() {
        assertEquals("[\"DICTATION\",\"INTERPRETATION\",\"REPEAT_AFTER_AUDIO\"]",
