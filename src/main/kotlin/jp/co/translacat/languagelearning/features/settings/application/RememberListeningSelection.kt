...
internal class RememberListeningSelection(private val unitOfWork: SelectionSettingsUnitOfWork) {
    suspend fun execute(userId: Long, eventId: Long, expectedRevision: LocalDateTime, taskTypes: List<ListeningTaskType?>): SelectionDeliveryStatus {
        require(userId > 0 && eventId > 0)
        require(expectedRevision.nano % 1000 == 0 && expectedRevision.year in 1000..9999) { "설정 revision은 마이크로초 정밀도여야 합니다." }
        ...
        return unitOfWork.execute selection@ {
            ...
            val saved = if (decision == SelectionDeliveryStatus.APPLIED) {
                ...
                saveIfChanged(current, current.copy(
