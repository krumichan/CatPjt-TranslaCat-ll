...
internal fun Route.settingsSelectionRoute(operation: RememberListeningSelection) {
    authenticate(INTERNAL_AUTH) {
        post("/internal/v1/language-learning/settings/listening-selection") {
            ...
            val revision = try { LocalDateTime.parse(request.expectedRevision) }
            catch (_: Exception) { throw LearningBusinessException("INVALID_REQUEST", "Settings revision 형식이 유효하지 않습니다.") }
            val types = try { request.taskTypes.map { it?.let(ListeningTaskType::valueOf) } }
            catch (_: IllegalArgumentException) { throw LearningBusinessException("INVALID_REQUEST", "Listening Task가 유효하지 않습니다.") }
